...
export function samePw(str,str1) {
...
export function checkPw(str,str1,callback){
    if(samePw(str,str1)===''&&checkPasswordPattern(str)===''){
            ...
            callback('pwCheck',true)
        
    }
    else{
        callback('pwCheck',false)
    }
}
export function checkEmailPattern(str) {
    var pass = str;
    var message = true;
    ...
    // 비밀번호 문자열에 특수문자 존재 여부 검사
    else if(pattern4.test(pass) == false) {
        alert('이메일 형식에 맞지 않습니다')
        message = false;
    }
...
export function testCheck(event,callback){
    console.dir(event.target.checked)
    if(event.target.checked)
    callback('admitCheck',true)
    else
    callback('admitCheck',false)
}
